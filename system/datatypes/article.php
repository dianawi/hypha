...
class article extends Page {
	...
	private $statusMtx = [
		self::STATUS_DRAFT => [self::STATUS_REVIEW => 'request review'],
	...
	protected function onStatusChange($statusArray) {
	...
	protected function onStatusChangeToPublish($statusArray) {
	...
	private function getDiscussions() {
		return $this->getContainerItems(self::FIELD_NAME_DISCUSSION_CONTAINER, self::FIELD_NAME_DISCUSSION);
	}
	...
	protected function processRequest() {
		...
		if (null !== $firstArgument && !isUser()) {
			notify('error', __('login-preform-action'));
	 ...
	 */
	private function ensureStructure($force = false) {
		if ($this->rootDocPage instanceof DocPage && !$force) {
			return false;
		}
		$this->xml->lockAndReload();

		/** @var HyphaDomElement $docElement */
		$docElement = $this->xml->documentElement;
		$new = $docElement->children()->count() === 0;
		if (!$new) {
		...
		$build($docElement, self::$dataStructure);
	...
	public function indexAction() {
		...
		if (isUser()) {
			$main->append($this->getReviewCommentsContainer());
	...
	private function getReviewCommentsContainer() {
		...
		return $reviewCommentsContainer;
	}
	...
	public function discussionResolvedAction($discussionId) {
		$this->lockAndReload();
...
EOF;
	 ...
	 * @param string $key
	 * @return array|DocPage|DocPage[]
	 */
	private function getContainerItems($container, $key) {
