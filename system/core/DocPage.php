...
class DocPage {
	...
	public static function build(Xml $xml) {
		...
		$pageDoc = $build($xml->documentElement);
		$pageDoc->setXml($xml);
	 ...
	 * @param string $name
	 */
	public function setAttrWithTimestamp($name) {
	...
	/**
	 * Locks the XML file at the root DocPage.
	 *
	 * After locking the file it reloads the file content and replaces the old docs with the new.
	 *
	 * @throws Exception
	 */
	public function lockAndReload() {
		$structureCurrent = $this->getMappedStructure();
		$root = $this->getRoot();
		$root->xml->lockAndReload();
		$structureAfterReload = self::build($root->xml)->getMappedStructure();
		if ($structureCurrent !== $structureAfterReload) {
			$root->xml->unlock();
			throw new \Exception('cannot reload structure');
		}

		$reload = function (DocPage $docPage, HyphaDomElement $doc) use (&$reload) {
			$docPage->doc = $doc;
			foreach ($docPage->children as $name => $childOrChildren) {
				$childNodes = $doc->getElementsByTagName($name);
				if ($childOrChildren instanceof DocPage) {
					$childOrChildren = [$childOrChildren];
				}
				foreach ($childOrChildren as $index => $child) {
					if ($childNodes->item($index) instanceof HyphaDomElement) {
						$reload($child, $childNodes->item($index));
					}
				}
			}
		};
		$reload($root, $root->xml->documentElement);
	}

	/**
	 * Saves the XML file and unlocks it.
	 */
	...
	/**
	 * Creates a serialized string of the current structure.
	 *
	 * @return string
	 */
	private function getMappedStructure() {
		$root = $this->getRoot();

		$getStructure = function (DocPage $docPage, HyphaDomElement $doc) use (&$getStructure) {
			$docPage->doc = $doc;
			...
			foreach ($docPage->children as $name => $childOrChildren) {
				...
				foreach ($childOrChildren as $index => $child) {
					if ($childNodes->item($index) instanceof HyphaDomElement) {
						$structure->{$name}[] = $getStructure($child, $childNodes->item($index));
					}
				}
			}
			return $structure;
		...
		return serialize($getStructure($root, $root->xml->documentElement));
	}
}
